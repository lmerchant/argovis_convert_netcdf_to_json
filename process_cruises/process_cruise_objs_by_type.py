...
from xarray_and_dask.explode_vars_with_extra_dim_to_cols import (
    explode_cdom_vars_to_cols,
)


def adjust_profiles_with_extra_dims_naming(
        ...
        if has_extra_dim:
            ...
            for key, val in extra_dim_obj.items():
                if key == "cdom":
                    ...
                    common_vars = cchdo_params_set.intersection(cdom_variables_set)
...
def combine_profiles(
    meta_profiles, all_data_profiles, cchdo_mapping_profiles, data_type
):
    #  https://stackoverflow.com/questions/5501810/join-two-lists-of-dictionaries-on-a-single-key

    profile_dict = defaultdict(dict)
    for elem in itertools.chain(
        meta_profiles, all_data_profiles, cchdo_mapping_profiles
    ):
        profile_dict[elem["station_cast"]].update(elem)
...
def create_profiles_objs(cruise_ddf_obj):
    ...
    for ddf_obj in ddf_objs:
        ...
        all_data_profiles, all_name_mapping = create_data_profiles(df_param)

        cchdo_mapping_profiles = create_cchdo_mappings(nc_mappings, all_name_mapping)
...
def process_cruise_objs_by_type(cruise_objs):
    ...
    for cruise_obj in cruise_objs:
        ...
        if not cruise_xr_obj["xr_file_objs"]:
            ...
            not_processed_log = (
                Path(GlobalVars.LOGGING_DIR) / "all_cruises_not_processed.txt"
            )
